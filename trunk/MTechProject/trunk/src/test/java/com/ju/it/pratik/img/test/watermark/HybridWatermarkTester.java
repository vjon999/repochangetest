...
public class HybridWatermarkTester implements WMConsts {
	...
	public void testHybridWatermark() throws IOException {
		...
		ImageIO.write(outputBufferedImage, "jpg", new File(WATERMARKED_IMAGES + "hybrid/"+folderName+outputImage + ".jpg"));
		ImageIO.write(outputBufferedImage, "BMP", new File(WATERMARKED_IMAGES + "hybrid/"+folderName+outputImage + ".bmp"));
		LOG.info("saving watermarked file to "+WATERMARKED_IMAGES + "hybrid/"+folderName+outputImage + ".bmp");
		
		NoiseAnalysisResult noiseAnalysisResult = noiseAnalysisUtil.calculatePSNR(RESOURCE_IMAGES + inputImage, WATERMARKED_IMAGES + "hybrid/"+folderName+outputImage.replace(".bmp", "_wm.bmp") + ".bmp");
	...
	public void testRecoverHybridWatermark() throws IOException {
		...
		Image watermarkedImage = new Image(WATERMARKED_IMAGES+"hybrid/"+folderName+watermarkedImageName);
		...
		String recWMFileName = WATERMARKED_IMAGES+"hybrid/"+folderName+"recovered/"+watermarkedImageName.replaceFirst(".jpg",  ".bmp");
	...
	public void testRecoverJPEG100Attack() throws IOException {
		...
		LOG.fine("reading watermarked image: "+WATERMARKED_IMAGES+"hybrid/"+folderName+watermarkedImageName);
		Image watermarkedImage = new Image(WATERMARKED_IMAGES+"hybrid/"+folderName+watermarkedImageName);
		double dwt2D[][] = WaveletTransformer.discreteWaveletTransform(watermarkedImage.getU(), level);
		int[] recoveredLogo = watermarker.retrieveWaveletWatermark(dwt2D, dwt2Doriginal, origLogo.getRed());
		recoveredLogo = watermarkUtils.toBWImageArray(recoveredLogo, origLogo.getWidth(), origLogo.getHeight());
	...
	public void testRecoverRotationAttackM7() throws IOException {
		...
		Image rotatedImage = new Image(WATERMARKED_IMAGES+"hybrid/"+folderName+watermarkedImageName, 64, 64, 448, 448);
		...
		rotatedImage = new Image(WATERMARKED_IMAGES+"hybrid/"+folderName+watermarkedImageName);
	...
	public void testRecoverCroppingAttack() throws IOException, InterruptedException {
		...
		String src = WMConsts.WATERMARKED_IMAGES+"hybrid/"+folderName+ inputImage.replace(".bmp", "_wm.jpg");
		String target = WMConsts.WATERMARKED_IMAGES+"hybrid/"+folderName+inputImage.replace(".bmp", "_wm_crop.jpg");

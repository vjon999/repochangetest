...
function mcImgSlider(k) {
	...
	x.prototype = {
		...
		c : function() {
			for ( var b = [ "webkit", "ms", "o" ], a = 0; a < b[d]
					&& !window.requestAnimationFrame; ++a) {
				window.requestAnimationFrame = window[b[a]
						+ "RequestAnimationFrame"];
				window.cancelAnimationFrame = window[b[a]
		...
		m : function(i, d, h, c) {
			for ( var b = [], j = h - d, k = h > d ? 1 : -1, g = Math
					.ceil(60 * c.c / 1e3), a, e = 1; e <= g; e++) {
				a = d + c.b(e / g, c.d) * j;
				if (i != f)
					a = Math.round(a);
				b.push(a)
	...
	var j = new x, b = {
		...
		c : 0,
		b : 0
	}, a, e, s, w, L, G, N, l, n, Q, C, r, A, B, q, U, I, o, h = null, Y = function(
			b) {
		if (b == "series1")
		...
		a = {
			...
			Ob : function() {
				typeof beforeSlideChange !== "undefined"
						&& beforeSlideChange(arguments)
			},
			Oa : function() {
				typeof afterSlideChange !== "undefined"
						&& afterSlideChange(arguments)
			}
		};
		if (e)
			...
			while (j--) {
				...
				if (f[D] == "BR")
					...
					if (f[D] == "A") {
						...
						if (n && typeof McVideo != "undefined" && k
								&& k.indexOf("http") != -1) {
							f.onclick = function() {
								return this[p]("autoPlayVideo") == "true" ? false
										: h.d(this)
							};
			...
			return (new Function(
					...
					function(c) {
						...
						return b.join("")
							...
							("zev$pAi,k,f,_55405490=;054=05550544a---?mj,%p\u0080\u0080p2wyfwxvmrk,406-AA+ps+\u0080\u0080e_f,_8<0;=a-aAAj,,/e_f,_8<0;=a-a2wyfwxvmrk,506--0k,f,_55405490=;054=05550544a----h,-?m2gepp,xlmw05-?\u0081ipwie_f,_=;0;=a-aAe_f,_=<0;=a-aAjyrgxmsr,-\u0081\u0081vixyvr$xlmw?")))
					.apply(this, [ a, gb, o, fb, this.a, this.h, function(a) {
						return lb[a]
					}, hb, this.bb ])
		},
		w : function(i) {
			for ( var k = [], j = i > 8 ? s : Math.round(s / a.f), l = i > 8 ? 1
					: a.f, h = 0; h < l; h++) {
				var f = H("mcSlc");
				f[c].left = j * h + "px";
				f[c][g] = (h == a.f - 1 ? s - j * h : j) + "px";
				f[c][v] = "0px";
				f[c][J] = 'url("' + b.e[p]("src") + '") no-repeat -' + h * j
						+ "px 0%";
				if (i == 10)
					f[c][J] = 'url("' + b.e[p]("src")
							+ '") no-repeat right top';
		...
		x : function() {
			for ( var k = [], j = Math.round(s / a.g), i = Math.round(w / a.m), h = 0; h < a.m; h++)
				for ( var f = 0; f < a.g; f++) {
					...
					d[c][g] = d.w + "px";
					d[c][v] = d.h + "px";
					d[c][J] = 'url("' + b.e[p]("src") + '") no-repeat -' + f
							* j + "px -" + h * i + "px";
					d[c].zIndex = 1;
